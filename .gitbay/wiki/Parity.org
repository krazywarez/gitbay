...
| delete, transfer, rename    | yes | no  | no  |
...
| email list, remove, primary | yes | yes | no  |
| dashboard aggregate         | yes | yes | yes |
| notification inbox          | yes | yes | yes |
| activity mail on, off       | yes | yes | no  |
...
a repository is also CLI-only, as is deleting an organization: each
removes or moves what clone URLs point at, and wants a typed command,
not a button.
