...
repository list. On the web the about text is its own tab: =/<owner>=
is the repository list, =/<owner>/-/about= the text, =/<owner>/-/activity=
the year of squares, and =/<owner>/-/people= an organization's members
and teams. A tab with nothing on it is not offered and its URL is a
404. The CLI's =profile show= is unchanged and still returns all of it
at once. The iOS client decodes and renders both formats,
