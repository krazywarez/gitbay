...
| request a review       | yes | yes | no  |
| choose body markup     | yes | yes | no  |
...
the web's create forms offer the choice, and the =no= above is the iOS
composer, which writes markdown. Diff-line comments have no format
column and are always markdown.
...
| fork                        | yes | yes | no  |
| pin                         | yes | yes | yes |
| bookmark, bookmark list     | yes | yes | no  |
...
| build cancel                | yes | yes | no  |
| dependency checks on/off    | yes | yes | no  |
| dependency status           | yes | yes | no  |
...
it — and the account settings page has the form; the =no= above is the
iOS client.
...
API token minting, deploy keys, account and instance administration. Deleting or transferring a repository is also
