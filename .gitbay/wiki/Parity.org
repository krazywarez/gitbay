...
release create and edit, the profile about text, and the file editor on
...
| preview profile about       | n/a | yes | no  |
...
=profile set= carries description, website, about and links; the JSON
API runs it like any other write. The account settings page has the form, and so does the iOS client:
