...
| revisions              | yes | yes | no  |
| range-diff             | yes | no  | no  |
...
=mr revisions= lists the heads a merge request has had, and the web
page lists them beside the reviews they staled; the iOS client shows
threads left on an earlier revision but not the heads. =mr range-diff=
compares two heads and has no web or iOS view. Batched review is not
built.
...
| milestone create, close, reopen | yes | no | no |
...
| bookmark list               | yes | yes | no  |
| watch, unwatch              | yes | yes | yes |
| mute                        | yes | no  | yes |
| settings, protection        | yes | yes | yes |
| require codeowners          | yes | yes | no  |
| access grants               | yes | no  | no  |
| webhooks                    | yes | no  | no  |
| import from a remote        | yes | no  | no  |
...
| release asset remove        | yes | no  | no  |
...
which the API carries as a JSON string.
...
of, nothing more. A mute wins over owning the repository or having
written the thread.
...
account and instance administration. Deleting or transferring a repository is also
CLI-only, as is deleting an organization: each wants a typed
