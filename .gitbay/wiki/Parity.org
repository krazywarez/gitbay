...
account and instance administration. Deleting, transferring or renaming
a repository is also CLI-only, as is deleting an organization and
pruning merge request heads (=admin mr prune=): each removes or moves
what clone URLs point at, and wants a typed command, not a button.
