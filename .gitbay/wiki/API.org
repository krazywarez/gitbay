...
on the MR head must be green, and a head whose =.gitbay/ci.yml= has a
job a push would run must carry some; a repository with no CI
configuration, or only scheduled and tag jobs, merges. Each
...
- merge requests: =mr.created=, =mr.edited=, =mr.commented=,
  ...
  to), =mr.milestoned= (milestone), =mr.merged= (number, sha),
  =mr.closed=
