...
=-image= sets the default image for jobs that name none
(=docker.io/library/debian:stable-slim= if unset); a job overrides it
with =image:= in =.gitbay/ci.yml=, validated as a reference so a config
file cannot turn it into podman arguments.
