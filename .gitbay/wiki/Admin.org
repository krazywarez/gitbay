...
runner attached to it alone. Nothing caps what an account queues: builds pending at
once and schedule intervals down to a minute are unbounded, and a
build nothing claims stays pending. Since a build runs only on a
runner its owner attaches, the cost is rows and the queue numbers on
=admin runners=, not compute; a per-account cap is krz/gitbay#206.
