...
polls over SSH with a key added by =keys add --scope runner=, which
reaches only the runner protocol and read-only git (a runner executes
arbitrary repository code, so the key it holds must not do more), then
clones, runs the steps, streams the log back and resolves the commit
status. Run it as a dedicated unprivileged user on a non-admin account.
=admin user create --key= registers a full-scope key, so the runner key
is added afterwards through a bootstrap key that is then removed:
...
the forge. A =.gitbay/ci.yml= in another repository there queues builds
no runner claims. Whether that changes is krz/gitbay#184.
...
=-cpus= and =-memory= cap one build's container (podman's own units,
e.g. =-cpus 2 -memory 4g=); unset means uncapped. They only take effect
where podman can create a cgroup for the container. Under rootless
podman with the cgroupfs manager, which is what a system service gets,
it cannot: the container runs inside the service's own cgroup and both
flags are accepted and ignored (krz/gitbay#188). Cap the unit instead.
bay1's drop-in sets =MemoryMax=6G= and =CPUQuota=300%= on a 7.7GB
four-core host with no swap: the memory cap is what keeps the forge
alive when a build allocates without bound, and it sits above the e2e
suite's 5GB peak rather than at a fair share. =OOMPolicy=continue= keeps
systemd from stopping the runner when a build is OOM-killed.
