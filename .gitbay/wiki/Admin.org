...
Builds currently run as the runner's own user with no container; the
service drop-in and =-repos= are the controls (krz/gitbay#144). The step
environment is constructed rather than inherited, so a build sees =PATH=,
=HOME= (its workspace), =LANG=, =CI=, its =GITBAY_*= variables and its
secrets and nothing else — but a step can still read what that user can
read, and concurrent builds share a =-workdir=.

Rootless podman is the chosen remedy; the host preparation ships ahead of
the runner that uses it, so the order is fixed:
