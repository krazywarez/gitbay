...
which is the build home — write an ssh config to
=$(getent passwd "$(id -u)" | cut -d: -f6)/.ssh/config=, or pass =-F=.
...
and still arrive); the account page has the same switch. Writing =@name= in an issue, merge request or
