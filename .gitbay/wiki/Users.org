...
mentioned), reviews, closes, or merges. =notifications settings mail
off= keeps the inbox and stops that mail (login links are not activity
and still arrive); the account page has the same switch.
...
- A list command prints one row per item, tab-separated, no header.
  ...
  may carry a word (=due 2027-01-01=, =via team=). The =gitbay= CLI pads
  the tabs into aligned columns when stdout is a terminal and leaves
  them as tabs when piped, so =cut -f= sees the same bytes stock ssh
  prints. Under =--json= nothing is touched.
