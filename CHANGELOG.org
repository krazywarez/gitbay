...
The CLI sends a leading =--term=<cols>[,color]= argument on the SSH
command line, since OpenSSH's multiplexed sessions do not forward a
session's =SetEnv=; the server strips it wherever it appears. Stock
ssh opts in with =ssh git@gitbay.org --term=120,color issue list
krz/gitbay= or =-o SetEnv=GITBAY_TERM=120,color=. Operators running
