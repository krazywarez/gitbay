...
- Side columns: state, labels and open milestones on the issue and
  ...
  settings and admin.
- Below 64rem the facet and section columns stack after their content,
  the dashboard's pinned repositories return to a chip row above the
  queues, and the navigator hides, since the tree page is the navigator
  on a phone.
