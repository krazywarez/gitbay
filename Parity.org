...
Wiki is the one row left where the web can do something no other
surface can: its handler reads the companion repository directly rather
than dispatching a command (krz/gitbay#48). Blame, file editing, log at
a ref, archive and the public listing were all in that state until
krz/gitbay!99 and !101 gave them commands.
