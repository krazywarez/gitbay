...
=build cancel= withdraws a queued build before a runner claims it; a
running one is the runner's to finish. Cancelling a duplicate of a
commit that already passed the job puts that result back on the commit.
No button on the build page yet.
