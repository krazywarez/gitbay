...
=search= is both plus issue and merge request titles, over everything
the caller can read; the web serves it at =/search= with a field in the
rail, and an anonymous visitor gets the public rows from the same query.
Titles only — body search wants FTS5 and belongs with krz/gitbay#114.
=repo grep= remains the per-repository file-contents search.
