...
Users with repo admin claim custom domains with =repo domain add=; ACME
issues certificates only for claimed hosts, so stray DNS pointed at the
server gets nothing. Claims are unverified in v1 — fine while
registration is closed; add DNS TXT verification before opening it.
