...
func main() {
	...
	root.AddCommand(
		...
		group("profile", "user and org profiles",
			...
			pass("set", "set your profile: [--description d] [--website url]", passOpts{server: []string{"profile", "set"}}),
		),
		webhookCmd(),
		remoteCmd(),
		initCmd(),
...
func repoCmd() *cobra.Command {
	return group("repo", "create and manage repositories",
		...
		group("domain", "custom domains for the pages branch",
			pass("add", "serve pages on a domain: <domain>", passOpts{server: []string{"repo", "domain", "add"}, needsRepo: true}),
...
func orgCmd() *cobra.Command {
	return group("org", "organizations",
		...
		pass("profile", "show or set an org profile: <org> [--description d] [--website url]", passOpts{server: []string{"org", "profile"}}),
